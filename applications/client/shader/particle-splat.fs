...
void main() {
    ...
    if(!rain){
        ...
        dist = clamp(len / radius, 0, 1);
    }
    else{
        if (len > radius * 0.8f) { discard; return; }
        dist = clamp(len / (radius * 0.8f), 0, 1);
    }
    ...
    if(rain && normalize(norm).y > 0.8f){
    //out_col = vec4(0.9,0.9,0.9,1);
    out_col *= ( fade_geom * 5.0f);
    }
}
