...
static bool gui_show_lensflare(true);
...
void gui_draw() {
    ...
    if (ImGui::BeginMainMenuBar()) {
        ...
        ImGui::Separator();
        ImGui::Separator();
        ImGui::Checkbox("Lensflare", &gui_show_lensflare);
        ImGui::Separator();
...
void gui_display_framebuffer(const Framebuffer& fbo) {
    ...
    for (uint32_t i = 0; i < fbo->color_textures.size(); ++i)
        if (ImGui::CollapsingHeader(std::string("color attachment " + std::to_string(i) + " (" + fbo->color_textures[i]->name + ")" + "##" + fbo->name).c_str()))
...
bool get_show_lensflare(){
    return gui_show_lensflare;
}