...
#include "globals.h"
#include "../../../applications/client/gui/IconFont.h"
#include <glm/glm.hpp>
#include <iostream>
...
Context::Context() {
    ...
    ImGui::GetStyle().ChildRounding = 0.75f * ImGui::GetStyle().WindowRounding;

    // HIDPI scaling
    //   https://decovar.dev/blog/2019/08/04/glfw-dear-imgui/#high-dpi

    // apply HIDPI_SCALE to the style -> this scales everything except font
    ImGuiStyle &style = ImGui::GetStyle();
    style.ScaleAllSizes(globals::HIDPI_SCALE);
    ...
    if (fs::exists(parameters.font_ttf_filename)) {
        ...
        ImGui::GetIO().FontDefault = ImGui::GetIO().Fonts->AddFontFromFileTTF(
                ...
                float(parameters.font_size_pixels) * globals::HIDPI_SCALE,
                &config);
    }else{
        std::cout << "No custom font given" << std::endl;
        ImFontConfig config;
        config.OversampleH = 3;
        config.OversampleV = 3;
        config.SizePixels = float(parameters.font_size_pixels) * globals::HIDPI_SCALE;
        ImGui::GetIO().Fonts->AddFontDefault(&config);
    }

    // load icon font
    if (fs::exists(parameters.icon_font_ttf_filename)) {
        //
        // merge icons into default font,
        // no change of font required
        //
        static const ImWchar icons_ranges[] = { ICON_MIN_FK, ICON_MAX_FK, 0 };
        ImFontConfig config;
        config.MergeMode = true;
        config.PixelSnapH = true;
        std::cout << "Loading: " << parameters.icon_font_ttf_filename << "..." << std::endl;
        ImGui::GetIO().Fonts->AddFontFromFileTTF(
                parameters.icon_font_ttf_filename.string().c_str(),
                float(parameters.font_size_pixels) * globals::HIDPI_SCALE,
                &config,
                icons_ranges );
    }else{
        std::cout << "No icon font given" << std::endl;
    }
